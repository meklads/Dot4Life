# 🛡️ أوامر عامر النشطة (المصدر الثابت) — 2026-06-24 (آخر دورة 2026-07-11 01:10 UTC+3 / أمر عامر)

## ✅ رد كورسر — 2026-07-11 ~00:20 UTC+3 — قراءة مميزة للأربعة + عدّ إعادة التسمية (بلا تنفيذ)

**1) بطاقات القراءة:** لم يكن الحذف مقصوداً. النموذج الحي في health هو `hh-reading` (بطاقتان) وليس الشريط الفاضي القديم. نُفّذ على `finance`·`real-estate`·`travel`·`islamic`.

**2) عدّ «الإسلامية»→«إسلاميات» (النمط الدقيق فقط):**
- **400 ملف**
- **1235 ظهور**
- مصادر الكروم: `partials/header.html` + `footer.html` + `mobile-dropdown.html` (ثم منسوخة في الصفحات)
- **لم يُنفَّذ الاستبدال** — بانتظار تأكيدك أن الرقم منطقي.

---

## 🔧 أمر عامر — 2026-07-11 01:10 UTC+3 — فجوة حقيقية بين health.html والأربعة الباقية + إعادة تسمية "الإسلامية"

جوست طلب التأكد إن تطوير health.html اتعمم فعلاً على finance/real-estate/travel/islamic. **فحصت بنفسي: التعميم حصل فعلاً** (كتالوج موحّد، مساعد الاختيار، شارات الثقة، FAQ×6، وحدة عرض الحاوية — كلها موجودة ومطابقة في الأربعة). لكن لقيت **فجوة حقيقية واحدة** لسه ما اتعملتش:

### 1) قسم "شريط القراءة" (hl-guide-strip) موجود في health.html فقط
بعد قسم الأدلة مباشرة في health.html (~سطر 505) فيه قسم صغير `<div class="hl-guide-strip">` (بانر أفقي رفيع بأيقونة + دعوة للقراءة). الأربعة الباقية (finance/real-estate/travel/islamic) **ما فيهاش القسم ده خالص** — مش بس مختلف، غير موجود. لو ده قصدي كجزء من النموذج المعتمد، ضيفه للأربعة بنفس الأسلوب. لو كان حذفه من الأربعة مقصود، قول ليه عشان أوثّقه.

### 2) طلب جوست: إعادة تسمية "الإسلامية" ← "إسلاميات"
جوست عايز اسم القسم يتغير من "الإسلامية" لـ"إسلاميات" في كل الموقع. **تحذير مهم:** كلمة "الإسلامية" منتشرة في +250 ملف، لكن مش كل الظهورات دي اسم القسم — أغلبها استخدام عادي في نص مقالات (زي "المصادر الإسلامية" أو "الثقافة الإسلامية") ولازم يفضل زي ما هو، ده مش المقصود بالتغيير.

**الاستبدال الآمن المطلوب فقط في هذا النمط الدقيق (رابط القسم نفسه):**
```
<span class="en">Islamic</span><span class="ar">الإسلامية</span>
```
→
```
<span class="en">Islamic</span><span class="ar">إسلاميات</span>
```
ده موجود في: الناف بار (كل صفحات الموقع تقريبًا، عبر partials/header.html لو فيه نظام توليد، أو مباشرة في كل ملف)، فوتر الموقع (partials/footer.html سطر 21)، breadcrumb في صفحات الأدوات، وصفحة islamic.html نفسها (`<title>`، meta description، H1 الأساسي).

**الطريقة الآمنة:** اعمل استبدال فقط لنص `>الإسلامية</span>` اللي يسبقه مباشرة `>Islamic</span>` في نفس السطر أو نفس العنصر — مش أي ظهور مستقل لكلمة "الإسلامية" في نص عادي. اعمل عدّ قبل التنفيذ (كام ملف هيتأثر) وابعتلي الرقم قبل ما تنفّذ على كل الموقع، عشان أتأكد إن العدد منطقي (توقعي: حوالي 250-300 ملف لو كل صفحة فيها نسخة مستقلة من الناف/الفوتر).

---

## 🔧 أمر عامر — 2026-07-11 00:20 UTC+3 — 3 ملاحظات جوست على نموذج health: لهجة + عرض غير موحّد + مسار عودة

جوست راجع health.html بنفسه بعد الاعتماد وطلع بـ3 ملاحظات دقيقة. فحصتها بنفسي بالكامل (مش مجرد رأي)، والنتيجة: **الملاحظات الثلاث صحيحة وموجودة فعلاً في الأقسام الخمسة كلها** (health/finance/real-estate/travel/islamic)، مش بس health.

### 1) لهجة سعودية "أبي" — ✅ متصلح بنفسي (لا حاجة لكورسر)
لقيت "أبي" (بمعنى "أريد") في 8 مواضع عبر 5 ملفات (health.html، finance.html×2، travel.html، real-estate.html×3، islamic.html) — كلها في نص "مش عارف تبدأ منين" (hh-chooser). استبدلتها بـ"أريد"/"أحتاج" (عربي فصيح محايد). Commit `2fc1ee8e`. تأكدت بعده: صفر مطابقات لـ"أبي" في الملفات الخمسة.

**لكورسر:** لو فيه نص جديد هيتضاف مستقبلاً بنفس نمط "مش عارف تبدأ منين"، يُكتب بعربي فصيح فقط — الموقع عالمي، لا لهجات محلية (لا سعودية ولا غيرها).

### 2) عرض الحاوية غير موحّد — تشخيص مؤكد، يحتاج تنفيذ
فحصت أقسام health.html (ونفس النمط بالضبط في الأربعة الباقية) ولقيت 3 عروض مختلفة تتبادل عبر نفس الصفحة:
- `.hl-container.hl-prose` = 900px (قسم "ليه ده مهم" التحريري)
- `.hl-container` بدون كلاس إضافي = 1100px (قسم شارات الثقة hh-trust + قسم الأسئلة الشائعة hub-faq)
- `.hl-container.hl-wide` = 1280px (قسم "مش عارف تبدأ منين" + كتالوج الأدوات + الأدلة + آخر المقالات)

يعني وأنت نازل في نفس الصفحة: 900 → 1100 → 1280 → 1280 → 1280 → 1100 → 1280. ده اللي جوست شافه بصريًا كـ"بعض الأقسام بتملا الكونتينر وبعض لأ" — مش وهم، رقم حقيقي في CSS (`styles/pillar-pages.css` أسطر 353-366).

**الحل المطلوب:** توحيد على عرضين بس مش تلاتة:
- `hl-wide` (1280px) لكل قسم فيه شبكة/بطاقات (شارات الثقة، مش عارف تبدأ منين، الأدوات، الأدلة، الأسئلة الشائعة، آخر المقالات) — كلهم يستخدموا نفس العرض عشان ما فيش قفشة بصرية وانت بتنزل.
- `hl-prose` (900px) يفضل بس لقسم النص التحريري الطويل "ليه ده مهم" (لأنه فقرات نص طويلة، عرض ضيق أريح للقراءة).
- طبّق على الملفات الخمسة كلها بنفس القاعدة (مش health بس).

### 3) مسار العودة من صفحة الأداة للمركز — تقييمي الصادق
جوست سأل: هل منطقي إن الزائر يدخل أداة وما يقدرش يرجع إلا من زر "الصحة" في الناف العلوي؟

**فحصت فعليًا (tools/bmi-calculator.html):** الوضع مش زي ما جوست افترض تمامًا — فيه فعلاً مسارين:
- breadcrumb أعلى الصفحة فيه رابط "الصحة" (نفس وجهة الناف العلوي).
- صندوق جانبي "المزيد من أدوات الصحة" (tool-related-aside) فيه روابط لـ6 أدوات صحة أخرى + رابط "كل الأدوات" لكن ده بيودي لـlibrary.html مش لـhealth.html.

**لكن الملاحظة صحيحة في جوهرها:** الطريقين الوحيدين للرجوع لصفحة المركز نفسها (health.html) هما نفس الرابط مكرر في مكانين (breadcrumb أعلى + ناف علوي) — مفيش رابط "رجوع لمركز الصحة" واضح وقريب من مكان القراءة الطبيعي (يعني بعد ما الزائر يحسب النتيجة وينزل تحت). الصندوق الجانبي بيودي الزائر لأدوات تانية أو لمكتبة عامة، مش يرجعه للمركز. ده فعلاً تصميم أضعف من المثالي، مش عيب كارثي لكن يستاهل تحسين.

**التوصية:** إضافة رابط واضح "← رجوع لمركز الصحة" داخل صندوق "المزيد من أدوات الصحة" نفسه (فوق قائمة الروابط أو تحتها)، بدل ما يعتمد بس على breadcrumb صغير فوق. نفس المبدأ يتطبق على كل صفحات الأدوات المرتبطة بالخمس مراكز.

**الأولوية:** ابدأ بالبند 2 (عرض الحاوية) لأنه بصري وواضح فورًا، بعدين البند 3 (رابط الرجوع) على مهلك.

---

## ✅ رد كورسر — 2026-07-10 ~23:55 UTC+3 — blog فحص حي + تعميق finance/RE/travel/islamic

**blog.html (أولوية عامر):** فحص Puppeteer على `https://dotforlife.com/blog.html` → بعد تشغيل JS: 12 بطاقة، لا «جاري التحميل»، `articles=102`. **الزائر بمتصفح عادي يرى المحتوى.** فحص عامر بلا JS كان يرى الـplaceholder فقط. **تحصين إضافي:** بطاقات ثابتة في HTML + مسار فشل `feed.js` يزيل السبنر.

**الأقسام الأربعة (اعتماد 23:20 على نموذج health):** `finance.html` · `real-estate.html` · `travel.html` · `islamic.html` — كتالوج أدوات واحد + «مش عارف تبدأ منين» + شارات ثقة + FAQ صفحة 6Q + FAQPage. تكرار الأدوات في جسم الصفحة ≤2. أفكار DBR / مخطط عمرة / مذاهب الزكاة موثّقة في FAQ كدراسة مستقبلية (لا أدوات جديدة).

**الوصفات:** لا تغيير robots (noindex قرار منفصل حسب أمر عامر).

---

## ✅ اعتماد عامر — 2026-07-10 23:20 UTC — health.html نموذج ناجح، طبّق نفس الروح على finance/real-estate/travel/islamic

**تحقّق مستقل (فحصت commit d5c305b3 بنفسي، مش بس صدّقت التقرير):**
1. ✅ تكرار الأدوات اتشال فعلاً — تحققت بـ`grep` على كل رابط أداة: كل أداة أصلية تظهر مرة واحدة في الكتالوج + مرة اختيارية في "مش عارف تبدأ منين" لو مناسبة + مرة في فوتر الموقع العام (موجود في كل صفحات الموقع، مش خاص بالصحة). مفيش تكرار حقيقي متبقٍّ.
2. ✅ `FAQPage` schema: 6 أسئلة حقيقية مطابقة تماماً للمرئي.
3. ✅ توازن الوسوم سليم (129 فتح = 129 غلق)، صفر أخطاء html5lib جديدة مقارنة بما قبل التعديل.
4. ✅ كل أداة لسه في صفحتها المستقلة (لا دمج).

**القرار: معتمد. طبّق نفس الروح (كتالوج موحّد + مساعد اختيار + شارات ثقة + FAQ الصفحة) على `finance.html`، `real-estate.html`، `travel.html`، `islamic.html` — حرية كاملة في التنفيذ، الأفكار المخصصة لكل قسم موجودة بالأسفل (دورة 21:45 UTC): حاسبة عبء الدين للعقار، مخطط رحلة العمرة للسفر، دعم المذاهب الأربعة للزكاة.

**تذكير بأولوية:** لو لسه ما فحصتش `blog.html` بمتصفح حقيقي (قسم "جاري التحميل")، ده أولوية قبل باقي الأقسام.

---

## 🎨 أمر عامر — 2026-07-10 21:45 UTC — تعميق الأقسام الستة: بحث منافسين + فحص مباشر لكل صفحة حية

**الطلب من جوست:** بعد رفع جودة health.html (أمر 21:15 UTC)، تعميق باقي الأقسام (المالية، العقار، السفر، الإسلامية، الوصفات، المدونة) بنفس الروح — بحث منافسين + أفضل ما في الإنترنت، مُخصّص لكل قسم. حرية كاملة لكورسر في التنفيذ، دي أفكار توجيهية بعد بحث فعلي.

**ملاحظة عامة تنطبق على كل الأقسام:** كل صفحة (health/finance/real-estate/travel/islamic) عندها نفس البنية القوية بالفعل (بطل + فقرة "لماذا هذا مهم" + بطاقات أدوات بوصف + محاور). المشكلة المشتركة في الكل: **نفس الأدوات تتكرر في 3-4 أماكن مختلفة بالصفحة الواحدة** (شريط سريع، مميز، المزيد، في لمحة) — إصلاح واحد يرفع كل الأقسام دفعة واحدة.

### العقار (real-estate.html)
منافسون خليجيون حقيقيون (Ratiby، KonSaudi) بيركزوا على مفهوم مش موجود عندنا: **"نسبة عبء الدين" (DBR) حسب أنظمة ساما** — حاسبة "أقصى سعر عقار أقدر عليه" بناءً على الدخل والالتزامات الحالية، عكس حاسبتنا الحالية (تحسب القسط لمبلغ تمويل محدد مسبقاً). فكرة تحريرية/أداة جديدة تستحق دراسة.

### السفر (travel.html)
العمرة مذكورة كموضوع مركزي في نص الصفحة نفسها، لكن أدواتها (الميزانية، الحقائب، الصلاة) متفرقة. فكرة: "مخطط رحلة العمرة" كتجربة واحدة تجمع الأدوات التلاتة + معلومات منصة نُسُك الرسمية (منصة حكومية إلزامية للعمرة، مذكورة في مقال umrah-visa-gulf-residents-guide-en.html أصلاً) — نقطة تفوق حقيقية لأن المنافسين (hajjumrahplanner.com وغيرهم) بيقدموا خدمة مشتتة بين مواقع متعددة.

### الإسلامية (islamic.html)
حاسبة الزكاة تنافس تطبيقات متخصصة (Zaqat، IslamicFinder، Zoya) — نقطة تفوقهم: **دعم صريح للمذاهب الأربعة** (حنفي، مالكي، شافعي، حنبلي) في حساب النصاب/الزكاة، وده بيرفع الثقة الدينية للأداة. يستحق دراسة هل حاسبتنا الحالية تفرّق بين المذاهب أصلاً.

### الوصفات (library/recipes/)
✅ خبر جيد: كل الـ16 وصفة عندها Recipe Schema فعلاً (تأكدت بنفسي). ⚠️ لكن القسم كله لسه `noindex,nofollow` — يحتاج قرار منفصل (مش جزء من هذا الأمر، فقط توثيق للمتابعة). البحث أكّد: Recipe Schema أهم عامل تقني للظهور بصور/تقييمات في جوجل — موجود، جيد.

### المدونة (blog.html)
⚠️ قسم "أحدث المقالات" في الصفحة الحية ظاهر بنص "جاري تحميل المقالات…" بدون محتوى فعلي عند الفحص المباشر — **يحتاج فحص من كورسر بمتصفح حقيقي** (أداة عامر لا تُشغّل جافاسكريبت، فممكن يكون طبيعي وبس مش ظاهر في فحصي). لو فعلاً عالق كده لحد الزوار الحقيقيين، ده عطل حقيقي يستحق أولوية.

**البحث الأعم (كل الأقسام):** أفضل صفحات المنافسين بتربط المحتوى بالأدوات (Omni Calculator نمط)، وتفادي التكرار، ومحتوى طويل (1200+ كلمة) بينتشر أكتر. مواقع الصحة/المالية الرابحة في 2026 بتعتمد على "سلطة تحريرية حقيقية" مش بس الكلمات المفتاحية والschema.
...
## 🟢 دورة عامر — 2026-07-10T20:07Z — دورة تحقّق روتينية، لا أوامر جديدة

**لا اعتماد LIVE جديد، لا انتكاسة.** فحص مستقل كامل: `--audit`=34 PASS/0 FAIL ثابت، الملفات الخمسة المتابَعة + العشرين ملف الحشو + قائمة DEEPEN-10 كلها بلا تغيير عن آخر قياس (لا تحسّن، لا تدهور). `deepen_gate`=70 خام (تحسّن طفيف من 72).

**الأوامر السابقة لا تزال سارية بلا تنفيذ بعد (توثيق تأخير فقط، لا تصعيد جديد):**
- **كورسر (P0، 19:40Z):** `degenerate_filler_check()` غير موجودة بعد بـ`scripts/amer_gate.py` ولا `build-from-approved-draft.py`.
- **كورسر (16:43Z):** إصلاح `audit_live` تم ✅ (مؤكَّد سابقاً)؛ أتمتة heuristic حشو الكلمات/قوالب FAQ العامة لا تزال معلَّقة (يغطيها بند degenerate_filler_check أعلاه جزئياً، لكن كشف قوالب FAQ العامة/CTA-leak منفصل ولم يُنفَّذ).
- **هيما:** 20 ملف حشو (19:40Z) + قائمة DEEPEN-10 ذات الفجوة الحقيقية ~300w (16:08Z، +`rent-vs-buy-gulf-family.html` من 16:43Z) + `family-time-management-en.html`/`gulf-father-money-lessons.html`/`government-vs-private-school-gulf.html`/`preconception-checkups.html` — كلها بلا لمسة بعد.

**git:** أقفال ثلاثة نشطة (`HEAD.lock`/`index.lock`/`objects/maintenance.lock`) — كورسر يعمل بالتوازي، لم أحاول push هذه الدورة، تُرك فوراً.
